...
export default function SignupForm() {
  ...
  const handleGoogleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(
        `You've signed up as a ${userType === "USER" ? "Job Seeker" : "HR Professional"}!`
      )
    } catch (error) {
  ...
  return (
    <form onSubmit={handleGoogleSignIn} className="space-y-6">
      <div className="space-y-4">
        <h3 className="text-lg font-semibold">Sign up with Google</h3>
        <p className="text-sm text-muted-foreground">
          Use your Google account to quickly create your Resumaid profile.
        </p>
      ...
      <div className="space-y-4 border-t border-border pt-6">
        <div>
          <h3 className="text-sm font-medium mb-3">I am a:</h3>
          <RadioGroup
            defaultValue="USER"
            value={userType}
        ...
        <div className="flex items-start space-x-2">
          ...
          <div className="grid gap-1.5 leading-none">
            ...
            <p className="text-xs text-muted-foreground">
              ...
              <CustomLink href="#" className="text-primary hover:underline">
                Terms of Service
              </CustomLink>{" "}
              and{" "}
              <CustomLink href="#" className="text-primary hover:underline">
...
function CustomLink({ href, className, children }:any) {
  return (
